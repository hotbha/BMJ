# Human Intervention Required — BookMyJuice
...
## H1 — Chargebee Site & API Key Setup
...
- `JWT_SECRET=BookMyJuiceSecureJWTKey2024Minimum32CharsRequired` (40 chars)
...
- `MAIL_PASSWORD=kr8vTh0gtAJb`
- `MAIL_FROM=support@bookmyjuice.co.in`

**Note:** Using Zoho SMTP instead of Gmail. The app password `kr8vTh0gtAJb` is configured.

**Remaining:** GitHub Actions secret `MAIL_PASSWORD` still needs to be created.

---

## H4 — Google OAuth Consent Screen & Client ID
...
## H5 — [RESOLVED] Stale architecture/status docs
...
## H6 — [RESOLVED] Hardcoded pricing page IDs
...
## H7 — [RESOLVED] Production Dockerfile review
...
## H8 — [RESOLVED] Flutter checkout screens
...
## H9 — Chargebee Java Library JARs
...
**Resolution:**
- `chargebee-java-3.29.0.zip` — Deleted
- `chargebee-java-3.30.0.zip` — Deleted
- `chargebee-java-3.30.0/` extracted directory — Deleted

The Chargebee Java SDK is included as a Maven dependency in `pom.xml`. No manual JARs needed.

---

## H10 — MySQL Database Initialization

**Status:** 🟡 Ready (Docker + Flyway)  
**Priority:** High  
**Owner:** DevOps
...
2. MySQL service is configured in `.env`:
   ...
   - `DB_USERNAME=bmj`, `DB_PASSWORD=PASS@123`
...
## H11 — Redis Setup (Production)

**Status:** 🟡 Partial (Docker available)  
**Priority:** Medium  
**Owner:** DevOps
...
- Note: `REDIS_HOST` and `REDIS_PORT` are not explicitly set in `.env` — add them if needed.

---

## H12 — GitHub Repository Secrets Configuration
...
|------------|--------|-------------|
| `CHARGEBEE_SITE` | Chargebee settings | ✅ Known (`bookmyjuice-test`) |
| `CHARGEBEE_API_KEY` | Chargebee settings | ✅ Known |
| `JWT_SECRET` | Generated (see H2) | ✅ Known |
| `WEBHOOK_USERNAME` | Custom | ✅ Known (`webhook_user`) |
| `WEBHOOK_PASSWORD` | Custom | ✅ Known |
| `MAIL_PASSWORD` | SMTP app password | ✅ Known |

All values exist in `.env` and just need to be copied to GitHub Secrets.

---

## H13 — [NEW] Webhook Username & Password Setup
...
- `WEBHOOK_PASSWORD=wh_secure_password_change_me`
...
## H14 — [NEW] JWT Expiration Override
...
## H15 — [RESOLVED] Health Endpoint Created
...
## H16 — [RESOLVED] Empty Cart Validation Fix
...
## H17 — [RESOLVED] Dynamic IP Handling for Phone Testing

**Status:** ✅ Resolved  
**Resolution:** Flutter `api_config.dart` updated to default to `10.0.2.2` (Android emulator). New helper scripts created in `ops/`:
- `find_active_ip.ps1` — Auto-detects your current WiFi IP
- `build_flutter_for_phone.ps1` — Builds Flutter APK with `--dart-define=API_BASE_URL=<your-ip>:8080`
- `test_fullstack_dynamic.ps1` — Full-stack test script with `-Remote` flag for phone testing

---

## H18 — [NEW] Firebase Project Configuration & google-services.json
...
- Go to [Firebase Console → Project Settings](https://console.firebase.google.com/project/bookmyjuice-4c156/settings/general)
- Under "Your apps" → Android app (`com.bookmyjuice.app`), click **"Download google-services.json"**
- **Replace** `lush/android/app/google-services.json` with the new file
- The current file has an empty `"oauth_client": []` array — the new file **must** contain OAuth client entries for Google Sign-In to work
...
- Go to [Firebase Console → Authentication → Sign-in method](https://console.firebase.google.com/project/bookmyjuice-4c156/authentication)
...
1. **🔧 CODE FIX (Applied):** The `_handleGoogleSignup()` method in `signup_method_selection_screen.dart` was firing **two** Google Sign-In attempts simultaneously — the BLoC `GoogleSignIn` event AND the direct `GoogleSignInHelper.instance.signIn()` call. Android's Credential Manager cannot handle concurrent auth requests, causing `[16] Account reauth failed.` The fix removes the redundant BLoC event, keeping only the single direct call.

2. **👤 HUMAN FIX NEEDED:** The `google-services.json` file has an empty `"oauth_client": []` array, so Android doesn't know which OAuth client to use. This causes the `GetCredentialResponse error returned from framework` log message. Re-downloading `google-services.json` from Firebase Console (after enabling Google Sign-In) will include the OAuth client IDs needed to validate tokens.
...
Files updated:
- `lush/lib/views/models/google_sign_in.dart` — `_serverClientId` changed to Web client ID
- `.env` — `GOOGLE_CLIENT_ID` and `GOOGLE_WEB_CLIENT_ID` updated
- `.env.example` — default updated
- `lush/web/index.html` — meta tag updated
- `bmjServer/src/main/resources/application.properties` — defaults updated
...
## H19 — [COMPLETED] Full-screen Theme Migration & Dashboard as Default
...
**Completed:**
1. ✅ **Dashboard as default (Requirement 2)**: Implemented `DashboardMode` enum, `AuthWrapper` rewritten to show public dashboard for unauthenticated users with login prompts on auth-gated actions. Compilation verified.
2. ✅ **Back button handling (Requirement 3)**: `PopScope` added to 11 critical screens with `BackButtonHandler` confirmation dialogs for in-progress operations. Compilation verified.
3. ✅ **Theme alignment (Requirement 1)**: All 14 remaining screen files migrated from legacy `LushTheme`/`FontUtils`/raw `Colors.*` to `AppColors`/`AppTextStyles`. Files migrated:
...
4. ✅ **`flutter analyze` passes with 0 errors** (all 14 files migrated, only pre-existing info-level style hints remain)
...
|----|------|--------|----------|
| H1 | Chargebee Site & API Key | ✅ Resolved | Critical |
| H2 | JWT Secret Generation | ✅ Resolved | Critical |
| H3 | SMTP Email Configuration | ✅ Resolved | High |
| H4 | Google OAuth Consent Screen | ✅ Resolved | High |
| H5 | Stale docs | ✅ Resolved | Medium |
| H6 | Hardcoded pricing page IDs | ✅ Resolved | Medium |
| H7 | Production Dockerfile | ✅ Resolved | Medium |
| H8 | Flutter checkout screens | ✅ Resolved | Medium |
| H9 | Chargebee Java Library JARs | ✅ Resolved | Low |
| H10 | MySQL Database Initialization | 🟡 Ready | High |
| H11 | Redis Setup | 🟡 Partial | Medium |
| H12 | GitHub Secrets Configuration | 🔴 Pending | High |
| H13 | Webhook Credentials | ✅ Resolved | Medium |
| H14 | JWT Expiration Override | ✅ Resolved | Low |
| H15 | Health Endpoint | ✅ Resolved | Low |
| H16 | Empty Cart Validation | ✅ Resolved | High |
| H17 | Dynamic IP Handling | ✅ Resolved | Medium |
| H18 | Firebase Configuration & google-services.json | ✅ Resolved | High |
| **H19** | **Full-screen Theme Migration & Dashboard as Default** | **✅ Completed** | **High** |
