"""
Debug: Dump Sign Up tab UI to see available elements.
"""
import time
import json
import os
...
try:
    ...
    print("=== On Login Page ===")
    # Find the Sign Up tab
    tabs = driver.find_elements(*uia('new UiSelector().className("android.view.View").descriptionContains("Sign Up")'))
    print(f"Found {len(tabs)} Sign Up elements")
    for t in tabs:
        desc = t.get_attribute('content-desc') or ''
        print(f"  desc='{desc}', clickable={t.get_attribute('clickable')}")
        t.click()
        break
    
    time.sleep(3)
    dump_page('08_signup_tab')
    
    # Now check what "Email" elements appear
    email_els = driver.find_elements(*uia('new UiSelector().descriptionContains("Email")'))
    print(f"\nEmail elements: {len(email_els)}")
    for el in email_els:
        print(f"  class={el.get_attribute('className')}, desc='{el.get_attribute('content-desc')}', clickable={el.get_attribute('clickable')}")
    
    # Also check "phone" elements
    phone_els = driver.find_elements(*uia('new UiSelector().descriptionContains("phone")'))
    print(f"Phone elements: {len(phone_els)}")
